...
class KerasFLModel(FLModel):
    ...
    def fit_model(self, train_data, fit_params=None):
        ...
        try:
            ...
            if type(train_data) is tuple and type(train_data[0]) is np.ndarray:
                ...
                self.fit_generator(
                    train_data, batch_size=32, epochs=epochs, steps_per_epoch=steps_per_epoch)
    ...
    def evaluate_model(self, x, y, batch_size=32, **kwargs):
        """
        ...
        with self.graph.as_default():
            set_session(self.sess)
            metrics = self.model.evaluate(x, y, batch_size=32, **kwargs)
